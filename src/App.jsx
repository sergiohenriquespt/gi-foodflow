...
// ─── Supabase helpers — mapeia colunas DB para campos da app ─────────────────
const mapFunc  = (r) => ({ ...r, foto: r.foto_url })
const mapEmenta = (r) => ({ ...r, data: r.data }) // data já vem como string YYYY-MM-DD
...
async function fetchEmentas()       { const { data } = await supabase.from('cantina_ementas').select('*').order('data').order('tipo'); return (data || []).map(mapEmenta) }
async function fetchMarcacoes()     { const { data } = await supabase.from('cantina_marcacoes').select('*'); return data || [] }
...
const Icon = ({ name, size=18, color='currentColor' }) => {
  const p = {
    ...
    spin:     <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round"><path d="M12 2a10 10 0 0 1 10 10"/></svg>,
  }
  return p[name] || null
}
...
function LoadingScreen({ msg='A carregar…' }) {
  return (
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
      <div style={{ width:48, height:48, borderRadius:'50%', border:`3px solid ${C.yellow}33`, borderTopColor:C.yellow, animation:'spin 0.8s linear infinite' }} />
...
function ErrorBanner({ msg, onRetry }) {
  return (
    <div style={{ background:C.dangerBg, border:`1px solid ${C.danger}44`, borderRadius:10, padding:'12px 16px', display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
                ...
                {serialStatus === 'error' && serialErrMsg && (
                  <div style={{ marginTop:8, fontSize:11, color:C.danger, textAlign:'center', lineHeight:1.4 }}>
                    {serialErrMsg.includes('already') || serialErrMsg.includes('open')
                      ? '⚠ Porta já em uso — outra tab pode estar a usá-la.'
                      : `⚠ ${serialErrMsg}`}
                  </div>
                )}
              </>
...
// ═════════════════════════════════════════════════════════════════════════════
function TerminalMarcacoes({ funcionarios, ementas, onBack }) {
  ...
  // Carrega marcações do funcionário autenticado
  const loadMarcacoes = async (funcId) => {
    const { data } = await supabase.from('cantina_marcacoes')
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.key === 'Enter') { if (rfidRef.current) { handleRfid(rfidRef.current); rfidRef.current = '' } return }
  ...
  useEffect(() => {
    ...
    const autoConnect = async () => {
      try { const ports = await navigator.serial.getPorts(); if (ports.length > 0 && isMounted.current) await openSerialPortM(ports[0]) } catch (_) {}
    }
    autoConnect()
    return () => { isMounted.current = false; closeSerialPortM() }
  }, [])
  ...
  const openSerialPortM = async (port) => {
    ...
    } catch (e) { if (isMounted.current) { setSerialStatus('error'); setSerialErrMsg(e?.message||'Erro ao abrir porta') } }
  }
  ...
  const startSerialReadingM = async (port) => {
    ...
    try {
      ...
      while (isMounted.current) {
        ...
        if (bufClean.length >= 4 && isMounted.current) { handleRfid(bufClean); buffer = '' }
      }
      reader.releaseLock()
  ...
  const closeSerialPortM = async () => {
    try { if (serialReader.current) { await serialReader.current.cancel(); serialReader.current = null } } catch (_) {}
    try { if (serialPort.current)   { await serialPort.current.close();    serialPort.current = null }   } catch (_) {}
  }

  const connectSerialM = async () => {
    if (!navigator.serial) return
    try { const port = await navigator.serial.requestPort(); await openSerialPortM(port)
    } catch (e) { if (e?.name !== 'NotFoundError' && isMounted.current) { setSerialStatus('error'); setSerialErrMsg(e?.message||'Porta não selecionada') } }
  }
  ...
  const handleRfid = (val) => {
    ...
    if (!f || !f.ativo) {
      setRfidMsg(`Cartão lido: ${uid} — não encontrado. Configura o UID na ficha.`)
  ...
  if (step === 'numero') return (
    <InputScreen title="Introduza o seu código de funcionário" subtitle="Código"
      ...
      onBack={onBack} confirmLabel="→" error={err}
      serialStatus={serialStatus} serialErrMsg={serialErrMsg} onConnectSerial={connectSerialM}>
      {rfidMsg && (
  ...
  const marcar = async (ementa, n) => {
    // Upsert: insere ou atualiza se já existe (unique funcionario_id + ementa_id)
    await supabase.from('cantina_marcacoes')
      .upsert({ funcionario_id:func.id, ementa_id:ementa.id, prato_num:n },
               { onConflict:'funcionario_id,ementa_id' })
    await loadMarcacoes(func.id)
  }

  const cancelar = async (eid) => {
    await supabase.from('cantina_marcacoes')
      .delete().eq('funcionario_id', func.id).eq('ementa_id', eid)
    await loadMarcacoes(func.id)
  }
...
// ═════════════════════════════════════════════════════════════════════════════
function TerminalValidacoes({ funcionarios, ementas, onBack }) {
  const [numInput,     setNumInput]     = useState('')
  ...
  useEffect(() => {
    ...
    window.addEventListener('keydown', handler)
    return () => { window.removeEventListener('keydown', handler); clearTimeout(rfidTimer.current) }
  }, [status, funcionarios, ementas])

  useEffect(() => {
    isMounted.current = true
    if (!navigator.serial) return
    const autoConnect = async () => {
      try { const ports = await navigator.serial.getPorts(); if (ports.length > 0 && isMounted.current) await openSerialPort(ports[0]) } catch (_) {}
    }
    autoConnect()
    return () => { isMounted.current = false; closeSerialPort() }
  }, [])
  ...
  const startSerialReading = async (port) => {
    ...
    try {
      ...
      while (isMounted.current) {
        ...
        const bufClean = buffer.replace(/[^\x21-\x7E]/g, '').trim()
        if (bufClean.length >= 4 && isMounted.current) { process(bufClean, true); buffer = '' }
      }
      reader.releaseLock()
    } catch (_) { if (isMounted.current) setSerialStatus('error') }
  }
  ...
  const connectSerial = async () => {
    if (!navigator.serial) return
    try { const port = await navigator.serial.requestPort(); await openSerialPort(port) } catch (_) {}
  }

  const reset = () => { setNumInput(''); setStatus(null); setManualMode(false) }

  const confirmarConsumo = async (func, ementa, pratoNum) => {
    const { data, error } = await supabase.from('cantina_consumos')
      .insert({ funcionario_id:func.id, ementa_id:ementa.id, prato_num:pratoNum })
      .select().single()
    if (error) return // unique constraint → já consumiu (tratado antes)
  ...
  const process = async (val, isRfid=false) => {
    const v = val.replace(/[^\x21-\x7E]/g, '').trim()
    if (!v) return
    setNumInput('')
    const func = isRfid
    ...
    // Verifica consumo existente no DB
    const { data: exConsumo } = await supabase.from('cantina_consumos')
      .select('prato_num').eq('funcionario_id', func.id).eq('ementa_id', ementa.id).maybeSingle()
    if (exConsumo) {
      const pk = `prato${exConsumo.prato_num}`
      setStatus({ type:'dup', func, pratoLabel:ementa[pk+'_label'], pratoDesc:ementa[pk+'_desc'] })
      setTimeout(reset, 6000); return
    }
    // Verifica marcação existente no DB
    const { data: marcacao } = await supabase.from('cantina_marcacoes')
      .select('prato_num').eq('funcionario_id', func.id).eq('ementa_id', ementa.id).maybeSingle()
    if (!marcacao) { setStatus({ type:'no-marc', func, ementa }); return }
    confirmarConsumo(func, ementa, marcacao.prato_num)
  }
  ...
  const renderResult = () => {
    ...
    if (status?.type === 'ok') {
      ...
      return (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:18, padding:'40px 32px', background:C.surface, border:`2px solid ${C.success}44`, borderRadius:24, textAlign:'center' }}>
    ...
    if (status?.type === 'dup') {
      const ps=pratoStyle(status.pratoLabel)
      return (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:18, padding:'40px 32px', background:C.surface, border:`2px solid ${C.warn}44`, borderRadius:24, textAlign:'center' }}>
          <Avatar nome={status.func.nome} foto={status.func.foto} size={100} />
          <div style={{ fontSize:28, fontWeight:800, color:C.text }}>{status.func.nome}</div>
    ...
    <div style={{ height:'100vh', background:C.bg, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, padding:'0 20px', height:56, display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          {!navigator.serial ? null : serialStatus==='connected' ? (
            <span style={{ fontSize:11, color:C.success, background:C.successBg, border:`1px solid ${C.success}33`, borderRadius:6, padding:'3px 10px', fontWeight:600, display:'flex', alignItems:'center', gap:5 }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:C.success, display:'inline-block' }}/> Leitor ligado
            </span>
          ) : serialStatus==='connecting' ? (
            ...
            <button onClick={connectSerial} style={{ fontSize:12, fontWeight:600, color:C.yellow, background:C.yellow+'18', border:`1px solid ${C.yellow}55`, borderRadius:8, padding:'5px 14px', height:34 }}>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        <div style={{ width:280, background:C.surface, borderLeft:`1px solid ${C.border}`, display:'flex', flexDirection:'column', overflowY:'auto', flexShrink:0 }}>
          ...
          {recentes.length===0
            ...
            : recentes.map((r,i) => {
              const ps=pratoStyle(r.pratoLabel), isFirst=i===0
              return (
...
// BACKOFFICE
// ═════════════════════════════════════════════════════════════════════════════
function Backoffice({ funcionarios, ementas, consumos, reload, onBack }) {
...
function SecEmentas({ ementas, reload }) {
  ...
  const add = (tipo) => {
    setEditing({ id:null, data:selD, tipo, prato1_label:'Carne', prato1_desc:'', prato2_label:'Peixe', prato2_desc:'', prato3_label:'Dieta', prato3_desc:'', prato4_label:'Vegetariano', prato4_desc:'' })
  }

  const save = async (e) => {
    setSaving(true)
    const { id, foto, ...row } = e // remove campos que não existem na tabela
    if (e.id) {
      await supabase.from('cantina_ementas').update(row).eq('id', e.id)
    } else {
      await supabase.from('cantina_ementas').insert(row)
    }
    await reload()
    setSaving(false)
    setEditing(null)
            ...
            <div key={tipo} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:14, marginBottom:10 }}>
              ...
              {em && <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7 }}>
                {[1,2,3,4].map(n => {
                  ...
                  return <div key={n} style={{ background:ps.bg, border:`1px solid ${ps.border}`, borderRadius:6, padding:'8px 10px' }}><div style={{ fontSize:13, fontWeight:700, color:ps.color }}>{l}</div><div style={{ fontSize:14, color:C.textSub, marginTop:2 }}>{d||<span style={{ color:C.textMuted, fontStyle:'italic' }}>sem descrição</span>}</div></div>
                })}
              </div>}
            </div>
          )
  ...
  return (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:22, maxWidth:580 }}>
      ...
      <div style={{ display:'flex', gap:8, justifyContent:'flex-end', marginTop:6 }}>
        ...
        <button onClick={()=>onSave(f)} disabled={saving} style={{ padding:'9px 22px', background:C.yellow, border:'none', borderRadius:8, fontSize:14, fontWeight:700, color:C.bg }}>
    ...
    try {
      ...
      try {
        while (true) {
          ...
          const bufClean = buffer.replace(/[^\x21-\x7E]/g, '').trim()
          if (bufClean.length >= 4) { clearTimeout(tid); found=true; set('rfid',bufClean); setRfidState('done'); setTimeout(()=>setRfidState('idle'),2500); return }
        }
  ...
  return (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:24, maxWidth:500 }}>
      ...
      <div style={{ marginBottom:6 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={readRfidCard} disabled={rfidState==='waiting'}
            style={{ flexShrink:0, height:40, padding:'0 14px', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer',
              ...
              color: rfidState==='done'?C.success:rfidState==='error'?C.danger:rfidState==='waiting'?C.textMuted:C.yellow, whiteSpace:'nowrap' }}>
      ...
      <div style={{ background:C.surface2, border:`1px solid ${C.border}`, borderRadius:8, padding:'9px 14px', marginBottom:16, fontSize:12, color:C.textMuted }}>
        PIN em branco → o funcionário entra diretamente com o código de utilizador.
      </div>
      ...
      <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
        ...
        <button onClick={handleSave} disabled={saving} style={{ padding:'8px 22px', background:C.yellow, border:'none', borderRadius:8, fontSize:14, fontWeight:700, color:C.bg }}>
          {saving ? 'A guardar…' : 'Guardar'}
        </button>
      </div>
    </div>
      ...
      <div style={{ background:C.surface, borderRadius:10, border:`1px solid ${C.border}`, overflow:'hidden' }}>
        {filtered.length===0 ? (
          <div style={{ padding:36, textAlign:'center', color:C.textMuted, fontSize:13 }}>
            {consumos.length===0?'Sem consumos registados.':'Sem registos para o filtro aplicado.'}
          </div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
...
export default function App() {
  ...
  const reload = useCallback(async () => {
    try {
      setLoadErr(null)
      const [f, e, c] = await Promise.all([fetchFuncionarios(), fetchEmentas(), fetchConsumos()])
      setFuncionarios(f)
      setEmentas(e)
      setConsumos(c)
    } catch (err) {
      setLoadErr('Erro ao carregar dados. Verifica a ligação.')
    }
  }, [])
  ...
  const shared = { funcionarios, ementas, consumos }

  if (mode === 'selector')   return <ModeSelector onSelect={setMode} />
  if (mode === 'marcacoes')  return (
    <div>
      {loadErr && <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:99 }}><ErrorBanner msg={loadErr} onRetry={reload}/></div>}
      <TerminalMarcacoes {...shared} onBack={()=>setMode('selector')} />
    </div>
  )
  if (mode === 'validacoes') return (
    <div>
      {loadErr && <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:99 }}><ErrorBanner msg={loadErr} onRetry={reload}/></div>}
      <TerminalValidacoes {...shared} onBack={()=>setMode('selector')} />
    </div>
  )
  if (mode === 'backoffice') return (
    <div>
      {loadErr && <ErrorBanner msg={loadErr} onRetry={reload}/>}
      <Backoffice {...shared} reload={reload} onBack={()=>setMode('selector')} />
    </div>
  )
}
