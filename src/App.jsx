...
const C = {
  ...
  yellow:'#e0cb4b', text:'#e2e8f0', textSub:'#94a3b8', textMuted:'#64748b',
...
const fetchFuncionarios  = async () => { const {data}=await supabase.from('cantina_funcionarios').select('*').order('numero'); return (data||[]).map(mapFunc) }
const fetchEmentas       = async () => { const {data}=await supabase.from('cantina_ementas').select('*').order('data').order('tipo'); return data||[] }
const fetchMarcacoesAll  = async () => { const {data}=await supabase.from('cantina_marcacoes').select('*').order('created_at'); return data||[] }
const fetchConsumos      = async () => { const {data}=await supabase.from('cantina_consumos').select('*').order('validado_em',{ascending:false}); return data||[] }
const fetchDefinicoes    = async () => {
  const {data}=await supabase.from('cantina_definicoes').select('*')
  const s={...DEFAULTS}; for(const r of data||[]) s[r.chave]=r.valor; return s
}
const saveDefinicao = async (chave,valor) =>
...
function Keypad({value,onChange,onConfirm,maxLen=10,confirmLabel='→',confirmDisabled=false}) {
...
// Ecrã de identificação (código ou PIN)
function InputScreen({title,subtitle,value,onChange,onConfirm,onBack,confirmLabel,error,secret=false,children,serialStatus,serialErrMsg,onConnectSerial}) {
  return (
